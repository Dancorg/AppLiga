...
test(`create match`, async ({ request }) => { // this test is too long and should be broken down into smaller tests but it covers the flow of creating a match which requires multiple steps and entities
  ...
  // get the token for the first user
  const loginResponse1 = await request.post(`${localhost}/api/auth/login`, {
    data: {
  ...
  // get the token for the second user
  const loginResponse2 = await request.post(`${localhost}/api/auth/login`, {
    data: {
  ...
  // get the token for the third user
  const loginResponse3 = await request.post(`${localhost}/api/auth/login`, {
    data: {
      email: `test3@example.com`,
      password: `password123`
    }
  });
  const { token: token3 } = await loginResponse3.json();

  // get the token for the fourth user
  const loginResponse4 = await request.post(`${localhost}/api/auth/login`, {
    data: {
      email: `test4@example.com`,
      password: `password123`
    }
  });
  const { token: token4 } = await loginResponse4.json();
  // get the token for the fourth user
  const loginResponse5 = await request.post(`${localhost}/api/auth/login`, {
    data: {
      email: `test5@example.com`,
      password: `password123`
    }
  });
  const { token: token5 } = await loginResponse5.json();
  // get the token for the fourth user
  const loginResponse6 = await request.post(`${localhost}/api/auth/login`, {
    data: {
      email: `test6@example.com`,
      password: `password123`
    }
  });
  const { token: token6 } = await loginResponse6.json();
...
test(`create and resolve match`, async ({request}) => {
  ...
  // Submit score
  const scoreResponse = await request.post(`${localhost}/api/matches/${matchResponseData.matchId}/score`, {
    data: {
      player1_hits: 3,
      player2_hits: 2
    }
